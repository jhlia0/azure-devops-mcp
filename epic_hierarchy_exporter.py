...
class EpicHierarchyExporter:
    ...
    async def get_epic_hierarchy(self, epic_id: int) -> Dict[str, Any]:
        ...
        if epic.work_item_type != "Epic":
            raise ValueError(f"Work item {epic_id} is not an Epic (type: {epic.work_item_type})")
        ...
        for feature in features:
            ...
            feature_hierarchy.append({
                "work_item": feature,
                "user_stories": user_story_hierarchy
            })
    ...
    async def _get_children_by_type(self, parent_id: int, work_item_type: str) -> List[WorkItem]:
        ...
        for i, feature_data in enumerate(features, 1):
            ...
            if user_stories:
                for j, us_data in enumerate(user_stories, 1):
                    ...
                    md_lines.append(f"**Assigned To:** {user_story.assigned_to or 'Unassigned'}")
                    ...
                    if tasks:
                        ...
                        for k, task in enumerate(tasks, 1):
                            ...
                            md_lines.append(f"  - **Assigned To:** {task.assigned_to or 'Unassigned'}")
                        md_lines.append("")
                    ...
                    if bugs:
                        ...
                        for k, bug in enumerate(bugs, 1):
                            ...
                            md_lines.append(f"  - **Assigned To:** {bug.assigned_to or 'Unassigned'}")
                        md_lines.append("")
            else:
